...
def extract_info(filename):
    ...
    separator_list = get_separator_list(extract_words(pdf.pages[0]), pdf.pages[0].width) # 左右分栏的情况
    if separator_list:
    ...
    if is_left:
        ...
        line_list = get_main_line_list([x for x in word_list if x["x0"] > separator])
        resume_info = split_main_resume(line_list,True,filename)
#         resume_info = crf_split([side_part_word_list]+line_list)
    elif is_right:
        print(f"separator为右模式下提取{filename}的简历信息")
        side_part_word_list = [x for x in word_list if x["x0"] > separator]
        line_list = get_main_line_list([x for x in word_list if x["x1"] < separator])
#         resume_info = crf_split([side_part_word_list] + line_list)
        resume_info = split_main_resume(line_list,True,filename)
#         resume_info = crf_split(line_list)
    else:
        side_part_word_list = [x for x in word_list if x["x1"] < separator]
        line_list = get_main_line_list([x for x in word_list if x["x0"] > separator])
        resume_info = split_main_resume(line_list,True,filename)
...
#         resume_info["side"] = []
    ...
    crf_resume_info = {}
#     for section in [ "edu", "job",'project']:
    for section in ["edu"]:
        if resume_info[section]:
            continue
        else:
            print(f"{filename} {section} 为空,尝试CRF提取")
            if not crf_resume_info:
                crf_resume_info = crf_split([side_part_word_list]+line_list)
            resume_info[section] = crf_resume_info[section]
    
    resume_info_base=resume_info['base']+resume_info['side']
    if not resume_info_base:
        print(f"{filename} base 为空,尝试CRF提取")
        if not crf_resume_info:
            crf_resume_info = crf_split([side_part_word_list]+line_list)
        resume_info['base'] = crf_resume_info['base']


    try:
        counter = Counter(x["size"] for x in word_list)
        normal_word_size = sorted(counter.items(), key=lambda x: x[1])[-1][0]  # sorted按照freq递增排序，[-1]定位到freq最高的size；，[0]定位到该size具体的值
        base_info = extract_base_info(resume_info, normal_word_size)
        final_res.update(base_info)
    except BaseException:
        print(f"提取{filename}的基本信息时出错，错误信息:")
        if (traceback.format_exc() not in traceback_list):
            traceback_list.append(traceback.format_exc())
            print(traceback.format_exc())
    try:
        edu_info = extract_edu_info(resume_info)
        final_res.update(edu_info)
    except BaseException:
        print(f"提取{filename}的教育信息时出错，错误信息:")
        if (traceback.format_exc() not in traceback_list):
            traceback_list.append(traceback.format_exc())
            print(traceback.format_exc())
    try:
        job_info = extract_job_info(resume_info)
        final_res.update(job_info)
    except BaseException:
        print(f"提取{filename}的工作信息时出错，错误信息:")
        if (traceback.format_exc() not in traceback_list):
            traceback_list.append(traceback.format_exc())
            print(traceback.format_exc())
    try:
        project_info = extract_project_info(resume_info)
        final_res.update(project_info)
    except BaseException:
        print(f"提取{filename}的项目信息时出错，错误信息:")
        if (traceback.format_exc() not in traceback_list):
            traceback_list.append(traceback.format_exc())
            print(traceback.format_exc())
...
def main(data_path):
    ...
    for filename in os.listdir(data_path):
        ...
        print(f"\r{num}", end="")
        try:
            output[filename.replace(".pdf", "")] = extract_info(data_path / filename)
        except BaseException:
            print(f"提取{filename}的简历信息发生未知错误，错误信息：")
            if (traceback.format_exc() not in traceback_list):
                traceback_list.append(traceback.format_exc())
                print(traceback.format_exc())

    with open(f"../prediction_result/test_result_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.json", "w") as f:
        json.dump(output, f)
...
if __name__ == "__main__":
    ...
    model.load_weights('../user_data/model_data/crf.h5')

    traceback_list = []
    if len(sys.argv) <= 1:
        data_path = Path("../data/Dataset/test_data")
    else:
